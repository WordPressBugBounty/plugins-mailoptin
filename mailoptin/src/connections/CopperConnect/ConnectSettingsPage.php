...
class ConnectSettingsPage extends AbstractCopperConnect
{
    ...
    public function connection_settings($arg)
    {
        ...
        if (self::is_connected()) {
            ...
            $description            = sprintf(__('Only re-authorize if you want to connect another Copper CRM account.', 'mailoptin'));
            $disconnect_integration = sprintf(
                ...
                wp_nonce_url(
                    add_query_arg('mo-integration-disconnect', 'copper', MAILOPTIN_CONNECTIONS_SETTINGS_PAGE),
            ...
            $description  = sprintf(__('Authorization is required to grant <strong>%s</strong> access to interact with your Copper CRM account.', 'mailoptin'), 'MailOptin');
        }

        $settingsArg[] = [
            'section_title_without_status' => __('Copper CRM', 'mailoptin'),
            'section_title'                => __('Copper CRM Connection', 'mailoptin') . " $status",
            'type'                         => self::EMAIL_MARKETING_TYPE,
            'logo_url'                     => MAILOPTIN_CONNECTION_ASSETS_URL . 'images/copper-integration.png',
            'copper_auth'                  => [
                'type'        => 'arbitrary',
                'data'        => sprintf(
                    '<div class="moBtncontainer"><a href="%s" class="mobutton mobtnPush %s">%s</a></div>%s',
                    $this->get_oauth_url('copper'),
                    $button_color,
                    $button_text,
                    $disconnect_integration
                ),
    ...
    public function remove_access_token_persistence($sanitized_data, $option_name)
    {
        ...
        if ($option_name == MAILOPTIN_CONNECTIONS_DB_OPTION_NAME) {
            unset($sanitized_data['copper_access_token']);
        }
    ...
    {
        if ( ! isset($_GET['mo-integration-disconnect']) || $_GET['mo-integration-disconnect'] != 'copper' || ! check_admin_referer('mo_disconnect_integration')) return;

        $old_data = get_option($option_name, []);
        unset($old_data['copper_access_token']);
    ...
    {
        if ( ! empty($_GET['mo-save-oauth-provider']) && $_GET['mo-save-oauth-provider'] == 'copper' && ! empty($_GET['access_token'])) {

            check_admin_referer('mo_save_oauth_credentials', 'moconnect_nonce');

            $old_data = get_option($option_name, []);
            $new_data = array_map('rawurldecode', [
                'copper_access_token'        => $_GET['access_token']
            ]);
    ...
    {
        if (MAILOPTIN_CONNECTIONS_DB_OPTION_NAME !== $option || ! isset($args['copper_auth'])) {
            return;
        }

        //Output error log link if  there is one
        echo self::get_optin_error_log_link('copper');
